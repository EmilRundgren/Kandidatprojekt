...
% Last Modified by GUIDE v2.5 08-May-2015 11:03:48
...
global RescaledImage voxel_size slice_resolution contrast nfile Regret Orginal file_list segment
segment = 0;
contrast = 1;
scale = 1.5;
...
if (choice == 1)
    ...
    if (PathName > 0)
        ...
        def = {'1'};
        choice2 = inputdlg('Ange ett jämnt heltal att skala ned bilden med (1 om skalning ej önskas)', 'Skalningsfaktor', 1, def);
        scale = str2double(choice2);
        if (isempty(scale))
            warndlg('Ingen skalning vald');
        elseif (~(rem(scale,2)==0)) && (scale ~= 1)
            warndlg('Inget jämnt heltal valt');
        else
            ...
            %else
                ...
                slice_resolution = slice_resolution/scale;
...
if (choice == 2)
    ...
    if (FileName == 0) 
        warndlg('Inget filnamn/filplacering valt');
    else
        def = {'1'};
        choice2 = inputdlg('Ange ett jämnt heltal att skala ned bilden med (1 om skalning ej önskas)', 'Skalningsfaktor', 1, def);
        scale = str2double(choice2);
        ...
        elseif (~(rem(scale,2)==0)) && (scale ~= 1)
            ...
            Regret = RescaledImage;
            Orginal = RescaledImage;
            maxresu = size(RescaledImage);
            maxres = maxresu(1);
            RescaledImage = RescaledImage(1:scale:maxres, 1:scale:maxres, :);
            size(RescaledImage)
            vol3dv2('cdata', RescaledImage, 'texture', '3D');
            colormap(jet(256));
            alphamap('rampup');
...
global RescaledImage nfile file_list
        ...
        if (PathName > 0)
            ...
            if (isempty(answer))
                ...
                for i=1:nfile
                    ...
                    dicomwrite(squeeze(RescaledImage(:,:,i)), baseFileName, dicominfo(file_list{i}), 'CreateMode', 'copy');
                end
            end
        end
    end
...
global RescaledImage Regret nfile slice_resolution voxel_size contrast segment
...
if (isempty(RescaledImage))
    ...
    if (contrast_new > 0)
        ...
        contrast = contrast*contrast_new;
    end
end
...
global RescaledImage Regret Orginal voxel_size slice_resolution contrast segment
...
%         PSNRfinal = PSNR/nfile;
    ...
    if (choice == 2)
        SSIM = 0;
                ...
                SSIMfinal = ssim(RescaledImage, Orginal);
        end
...
global segment RescaledImage voxel_size slice_resolution contrast

if (segment == 0)
    warndlg('Segmentering krävs för att klippa ut ett segment')
else
    antal = max(max(max(RescaledImage)));
    antalstr = int2str(antal);
    inputtxt = strcat('Ange label som önskas att klippa ut, antalet labels är:',antalstr);
    def = {'1'};
    choice = inputdlg(inputtxt, 'Labelnummer', 1, def);
    labelnum = str2double(choice);
    if (labelnum == 0)
        warndlg('Ingen eller ej existerande label vald')
    elseif (labelnum > antal)
        warndlg('Ej existerande label vald')
    elseif (isempty(labelnum))
        warndlg('Ingen label vald')
    else
%         g = figure('units','normalized','outerposition',[0 0 1 1]);
        RescaledImage = RescaledImage==labelnum;
        cla
        vol3dv2('cdata', RescaledImage, 'texture', '3D');
        colormap(jet(256));
        alphamap('rampup');
